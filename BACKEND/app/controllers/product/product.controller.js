...
exports.create = async (req, res, next) => {
  if (req.body.length === 0) {
    res.send("Data create can not be empty");
  }
  try {
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      const fileName = parts[parts.length - 1];
      fs.unlinkSync(
        path.join(__dirname, `../../store/img/product/${fileName}`)
      );
    ...
    return next(
      new ApiError(
        500,
        `Error updating Product with id=${req.params.id} ${error}`
      )
    );
  }
};
...
exports.getProductImg = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(500, `Error retrieving Product with id = ${error}`)
    );
  }
};
