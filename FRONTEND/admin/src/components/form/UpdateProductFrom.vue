<template>
  <div class="col-sm-12 col-md-6 border border-dark rounded mx-auto my-4">
    <h4 class="p-2 text-organ text-center border-bottom border-dark">
      {{ Add ? "Thêm sản phẩm" : "Cập nhật sản phẩm" }}
    </h4>
    <strong v-if="isShowUpdateSuccess" class="update-success">{{
      Add ? `Thêm thành công x  ${CountAddProduct}` : "Cập nhật thành công"
    }}</strong>
    <Form
      :validation-schema="ProductAddValidate"
      @submit="handleSubmit"
      ...
      <table class="table table-hover mb-0">
        ...
        <tr>
          ...
          <td>
            ...
            <Field
              type="hidden"
              name="desPathUpload"
              v-model="ProductData.desPathUpload"
            />
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            ...
            <p class="m-0">
              <ErrorMessage name="img" class="text-danger error-message" />
            </p>
          </td>
        </tr>
